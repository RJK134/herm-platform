// prisma/seeds/fhe-framework-data.ts
...
//
// This file lives in prisma/seeds/ rather than server/src/data/ so the
// seeder (run via tsx + ESM) can resolve it without crossing the
// workspace tsconfig boundary. Phase 14.1's seed had to wrap the FHE
// seeder in a try/catch because cross-tree imports failed at runtime
// on some Node 22 setups (tsx's ESM resolution didn't reliably handle
// extensionless imports across workspace boundaries). Phase 14.7
// fixes that by colocating the data with the seed function that
// consumes it. The server runtime does not import this data — it's
// seed-only — so the move is safe.
