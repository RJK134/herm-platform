...
describe('buildSystemPrompt', () => {
  ...
  it('uses the non-HERM name when FHE is active (regression for Bugbot "HERM coverage" finding)', () => {
    ...
    // The product branding line is fixed ("HERM Procurement Assistant" is the
    // product name); what must be dynamic is the *active framework* label.
    expect(prompt).toContain('HERM Procurement Assistant');
  });
