...
export function AdminSystems() {
  ...
  return (
    <div>
      <Header
        title={t('systems.title', 'Admin — Systems')}
        subtitle={t('systems.subtitle', 'Managing {{count}} vendor systems in the HERM database', { count: systems?.length || 0 })}
      />
      ...
      <Card className="mb-4">
        <div className="flex gap-3 items-center flex-wrap">
          <div className="flex-1 min-w-[200px]">
            <SearchInput value={search} onChange={setSearch} placeholder={t('systems.searchPlaceholder', 'Search systems or vendors...')} />
          </div>
          <select
            value={category}
          ...
          >
            <option value="">{t('systems.allCategories', 'All Categories')}</option>
            {categories.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          <Button onClick={() => setShowAdd(true)}>
            <Plus className="w-4 h-4 mr-1" /> {t('systems.addSystem', 'Add System')}
          </Button>
        </div>
      </Card>
      ...
      <Card className="p-0 overflow-hidden">
        {isLoading ? (
          <div className="p-12 text-center text-gray-400">{t('systems.loading', 'Loading systems...')}</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                {[t('systems.colSystem', 'System'), t('systems.colVendor', 'Vendor'), t('systems.colCategory', 'Category'), t('systems.colCloud', 'Cloud'), t('systems.colRegions', 'Regions'), t('systems.colOwnSystem', 'Own System'), t('systems.colActions', 'Actions')].map(h => (
            ...
            <tbody>
              {filtered.map(system => (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    {system.cloudNative ? (
                      <span className="flex items-center gap-1 text-green-600 text-xs font-medium">
                        <CloudLightning className="w-3.5 h-3.5" /> {t('systems.native', 'Native')}
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-gray-400 text-xs">
                        <Globe className="w-3.5 h-3.5" /> {t('systems.onPrem', 'On-prem')}
                      </span>
                    )}
                  </td>
                  <td className="py-3 px-4">
                  ...
                  <td className="py-3 px-4">
                    {system.isOwnSystem && (
                      <span className="text-xs bg-teal text-white px-2 py-0.5 rounded font-semibold">{t('systems.yes', 'YES')}</span>
                    )}
                  </td>
                  <td className="py-3 px-4">
                    <Button size="sm" variant="ghost" onClick={() => setViewSystem(system)}>
                      <Eye className="w-3.5 h-3.5 mr-1" /> {t('systems.view', 'View')}
                    </Button>
                  </td>
                </tr>
              ))}
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-2">
              ...
              {viewSystem.isOwnSystem && (
                <span className="text-xs bg-teal text-white px-2 py-0.5 rounded font-semibold">{t('systems.yourSystem', 'YOUR SYSTEM')}</span>
              )}
              {viewSystem.cloudNative && (
                <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">{t('systems.cloudNative', 'Cloud Native')}</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
              <div>
                <div className="text-gray-400 text-xs mb-0.5">{t('systems.colVendor', 'Vendor')}</div>
                <div className="font-medium dark:text-white">{viewSystem.vendor}</div>
              </div>
              <div>
                <div className="text-gray-400 text-xs mb-0.5">{t('systems.colCategory', 'Category')}</div>
                <div className="font-medium dark:text-white">{viewSystem.category}</div>
              </div>
              <div className="col-span-2">
                <div className="text-gray-400 text-xs mb-0.5">{t('systems.colRegions', 'Regions')}</div>
      ...
      {/* Add system modal */}
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title={t('systems.addNewSystem', 'Add New System')}>
